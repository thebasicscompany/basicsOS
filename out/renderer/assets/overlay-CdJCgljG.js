import { r as reactExports, M as MotionConfigContext, j as jsxRuntimeExports, i as isHTMLElement, u as useConstant, P as PresenceContext, a as usePresence, b as useIsomorphicLayoutEffect, L as LayoutGroupContext, m as motion, c as clientExports } from "./proxy-CG65NCoh.js";
...
const log$2 = createOverlayLogger("whisper");
let audioCtx = null;
...
const VAD_SILENCE_THRESHOLD = 8e-3;
...
const useSpeechRecognition = (options) => {
  ...
  const startListening = reactExports.useCallback(() => {
    ...
    navigator.mediaDevices.getUserMedia({
      ...
      log$2.error("getUserMedia failed:", msg);
      setIsListening(false);
      setInterimText(msg);
    });
  ...
  const stopListening = reactExports.useCallback(async () => {
    ...
    try {
      ...
      log$2.error("Transcription error:", err);
    }
    setTranscript(text);
    setInterimText("");
    return text;
...
const useMeetingRecorder = (_chunkIntervalMs, onError) => {
  ...
  const startRecording = reactExports.useCallback(async (mid) => {
...
const log$1 = createOverlayLogger("ai-response");
...
let simIdx = 0;
const getSimulatedResponse = (transcript) => {
...
const streamAssistantAPI = async (message, onToken, onComplete) => {
  try {
    ...
    if (!apiUrl || !sessionToken) {
      log$1.warn("No API URL or session token — falling back to simulated");
...
const CompanyLogo = ({ logoUrl }) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    motion.div,
    {
      ...
      children: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { width: "15", height: "15", viewBox: "0 0 20 20", fill: "none", children: [
              ...
              children: [
                pill.state === "idle" && !flash.message && /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "div",
                  {
                    ...
                    children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx(CompanyLogo, { logoUrl: branding?.logoUrl ?? null }),
