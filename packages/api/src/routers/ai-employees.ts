...
export const aiEmployeesRouter = router({
  listJobs: protectedProcedure.query(async ({ ctx }) => {
    ...
    return ctx.db.select().from(aiEmployeeJobs)
      .where(eq(aiEmployeeJobs.tenantId, ctx.tenantId));
  }),
  ...
  getJob: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      if (!ctx.tenantId) throw new TRPCError({ code: "UNAUTHORIZED" });
      const [job] = await ctx.db.select().from(aiEmployeeJobs)
        .where(and(eq(aiEmployeeJobs.id, input.id), eq(aiEmployeeJobs.tenantId, ctx.tenantId)));
      if (!job) throw new TRPCError({ code: "NOT_FOUND" });
      const outputs = await ctx.db.select().from(aiEmployeeOutputs)
  ...
  createJob: memberProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const [job] = await ctx.db.insert(aiEmployeeJobs)
        .values({
  ...
  kill: adminProcedure
    .input(z.object({ id: z.string().uuid() }))
    .mutation(async ({ ctx, input }) => {
      const [updated] = await ctx.db.update(aiEmployeeJobs)
        .set({ status: "killed" })
        .where(and(eq(aiEmployeeJobs.id, input.id), eq(aiEmployeeJobs.tenantId, ctx.tenantId)))
        .returning();
  ...
  approveOutput: memberProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const [output] = await ctx.db.select().from(aiEmployeeOutputs)
      ...
      const [approved] = await ctx.db.update(aiEmployeeOutputs)
  ...
  listOutputs: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      if (!ctx.tenantId) throw new TRPCError({ code: "UNAUTHORIZED" });
      const [job] = await ctx.db.select({ id: aiEmployeeJobs.id }).from(aiEmployeeJobs)
        .where(and(eq(aiEmployeeJobs.id, input.jobId), eq(aiEmployeeJobs.tenantId, ctx.tenantId)));
      if (!job) throw new TRPCError({ code: "NOT_FOUND" });
      return ctx.db.select().from(aiEmployeeOutputs)
