...
export const automationsRouter = router({
  ...
  get: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      if (!ctx.tenantId) throw new TRPCError({ code: "UNAUTHORIZED" });
      const [automation] = await ctx.db.select().from(automations)
  ...
  create: memberProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const [automation] = await ctx.db.insert(automations)
  ...
  update: memberProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [updated] = await ctx.db.update(automations)
        .set(updateData)
  ...
  delete: memberProcedure
    .input(z.object({ id: z.string().uuid() }))
    .mutation(async ({ ctx, input }) => {
      const [deleted] = await ctx.db.delete(automations)
        .where(and(eq(automations.id, input.id), eq(automations.tenantId, ctx.tenantId)))
        .returning();
  ...
  listRuns: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const [automation] = await ctx.db.select({ id: automations.id }).from(automations)
        .where(and(eq(automations.id, input.automationId), eq(automations.tenantId, ctx.tenantId)));
      if (!automation) throw new TRPCError({ code: "NOT_FOUND" });
      return ctx.db.select().from(automationRuns)
