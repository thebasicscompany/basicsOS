...
async function ensureAdminUser(db) {
    ...
    if (res.ok) {
        const [crmUser] = await db
            ...
            .limit(1);
        if (!crmUser)
            throw new Error("CRM user not found after signup");
        return crmUser.id;
    }
...
async function main() {
    if (process.env.NODE_ENV === "production") {
        log.error("Refusing to seed in production. Seed creates admin@example.com with a known password. " +
