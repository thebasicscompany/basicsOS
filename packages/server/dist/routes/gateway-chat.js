...
import { BASE_SYSTEM_PROMPT, OPENAI_TOOL_DEFS, buildRecentConversationContext, extractRecordReferences, finalizeAssistantText, isRecordAdviceQuery, requestSchema, resolveThreadEntityReference, sdkPart, toolFallbackText, toOpenAIMessages, updateThreadEntityMemory, } from "../routes/gateway-chat/protocol.js";
import { ensureThread, getThreadEntityMemory, getThreadMessages, persistMessage, saveThreadEntityMemory, updateThreadTitle, touchThread, } from "../routes/gateway-chat/storage.js";
import { executeValidatedTool } from "../routes/gateway-chat/tools.js";
import { isLookupFailure, isLookupTool, planToolWorkflow, resolveDeferredToolStep, shouldPlanToolWorkflow, } from "../routes/gateway-chat/workflow.js";
import { routeChatRequest } from "../routes/gateway-chat/router.js";
const MAX_HISTORY_MESSAGES = 12;
const MAX_HISTORY_CHARS = 16_000;
const MAX_MESSAGE_CHARS = 4_000;
const MAX_TOOL_ARGUMENTS_CHARS = 4_000;
const VALID_GATEWAY_TOOL_NAMES = new Set(OPENAI_TOOL_DEFS.map((definition) => definition.function.name));
const WIKI_TOKEN_RE = /\[\[([a-z][a-z0-9-]*)\/(\d+)\|([^\]]+)\]\]/g;
function stripTokens(text) {
    return text.replace(WIKI_TOKEN_RE, (_m, _slug, id, name) => `${name} (id: ${id})`);
}
...
function truncateText(text, maxChars = MAX_MESSAGE_CHARS) {
...
function sanitizeGatewayToolCalls(value) {
    if (!Array.isArray(value)) {
        return { toolCalls: [], invalidToolNames: [] };
    }
    const invalidToolNames = new Set();
...
function buildInvalidToolRetryMessage(invalidToolNames) {
    return [
...
function sanitizeAssistantGatewayMessage(aiMessage) {
    ...
    const googleExtra = aiMessage?.extra_content &&
        typeof aiMessage.extra_content === "object" &&
        aiMessage.extra_content.google &&
        typeof aiMessage.extra_content.google ===
            "object"
        ? aiMessage.extra_content.google
        : null;
...
function inferWorkflowHints(queryText) {
    const trimmed = queryText.trim();
    const lower = trimmed.toLowerCase();
    const nextHintByTool = {};
    const planLines = [
    ...
    const isCreateContact = /\b(create|add|make)\b/.test(lower) &&
        /\b(contact|person|lead)\b/.test(lower);
    const isCreateDeal = /\b(create|add|make)\b/.test(lower) &&
    ...
    if (isUpdate && entity) {
        ...
        planLines.push(`For this request, the required sequence is: identify the ${entity.label} with \`${entity.searchTool}\` if needed, then call \`${updateTool}\`, then confirm the completed change.`, `Apply the requested changes from the user's original request exactly: "${trimmed}".`);
    ...
    if (isTask && /\bcontact|contacts|person|people|lead|leads\b/.test(lower)) {
        planLines.push("For task requests about a person/contact, the usual sequence is: identify the contact first if needed, then call `create_task`, then confirm the task was created.");
    ...
    if (isNote && /\bdeal|deals|opportunity|opportunities\b/.test(lower)) {
        planLines.push("For note requests about a deal, the usual sequence is: identify the deal first if needed, then call `add_note`, then confirm the note was added.");
        ...
        /\bcontact|contacts|person|people|lead|leads\b/.test(lower)) {
        planLines.push("For note requests about a contact, the usual sequence is: identify the contact first if needed, then call `add_note`, then confirm the note was added.");
    ...
    if (isCreateContact && /\bcompany|companies|organization\b/.test(lower)) {
        planLines.push("When creating a contact linked to a company, identify the company first if needed, then call `create_contact` with the company id/name, then confirm the contact was created.");
    ...
    if (isCreateDeal && /\bcompany|companies|organization\b/.test(lower)) {
        planLines.push("When creating a deal linked to a company, identify the company first if needed, then call `create_deal` with the company id/name, then confirm the deal was created.");
...
function duplicateToolMessage(name, nextStepHint) {
...
function buildToolChatContent(name, raw, nextStepHint) {
...
function buildResolvedRecordContext(record) {
    return [
...
function buildAmbiguousRecordReply(candidates, entity) {
...
function getLookupToolForRecord(record) {
...
async function synthesizeFinalAnswer(args) {
    ...
    return (finalizeAssistantText(json.choices?.[0]?.message?.content ?? "", [])
        || toolFallbackText(args.toolOutputs));
}
/** Shared chat turn logic used by both gateway-chat (UI) and stream-assistant (voice). */
export async function processChatTurn(db, env, params) {
    const { crmUser, gatewayHeaders, gatewayUrl, messages: uiMessages, threadId: threadIdParam, channel = "chat", } = params;
    ...
    let threadEntityMemory = await getThreadEntityMemory(db, threadId, crmUser.organizationId);
    ...
    const resolvedRecentRecord = resolveThreadEntityReference(queryText, threadEntityMemory);
    if (!threadIdParam?.trim()) {
        const fallback = queryText.length > 80 ? queryText.slice(0, 77) + "..." : queryText;
        await updateThreadTitle(db, threadId, fallback);
        if (routingDecision.shouldGenerateTitle) {
            generateSmartTitle(db, threadId, queryText, gatewayHeaders, gatewayUrl).catch(() => { });
        }
    }
    ...
    if (routingDecision.mode === "crm_context" ||
        ...
        const [crmSummary, { crmContext, meetingContext }] = await Promise.all([
            ...
            retrieveDualContext(db, gatewayUrl, gatewayHeaders, crmUser.organizationId, queryText, crmUser.id),
        ]);
        systemPrompt += `\n\n## Your CRM\n${crmSummary}`;
        if (crmContext)
            systemPrompt += `\n\n## Relevant context\n${crmContext}`;
    ...
    if (resolvedRecentRecord.mode === "resolved" &&
        routingDecision.mode === "tool_call") {
    ...
    if (!finalContent &&
        ...
        finalContent = buildAmbiguousRecordReply(resolvedRecentRecord.candidates, resolvedRecentRecord.entity);
    }
    if (!finalContent &&
        toolsEnabled &&
        ...
        const result = await executeValidatedTool(db, crmUser.id, crmUser.organizationId, toolName, toolArgs, {
            gatewayUrl,
            gatewayHeaders,
        ...
        if (typeof result === "string") {
            threadEntityMemory = updateThreadEntityMemory(threadEntityMemory, extractRecordReferences(result));
        }
    ...
    if (!finalContent && toolsEnabled && shouldPlanToolWorkflow(queryText)) {
        ...
        if (plannedWorkflow?.mode === "multi_tool" &&
    ...
    for (let i = 0; i < maxToolRounds && !finalContent; i++) {
        ...
        if (!res.ok) {
            ...
            throw Object.assign(new Error(`Gateway error ${res.status}`), { status: 502, details: errText.slice(0, 400) });
        }
        const json = (await res.json());
        const aiMessage = json.choices?.[0]?.message;
        const { toolCalls, invalidToolNames } = sanitizeGatewayToolCalls(aiMessage?.tool_calls);
    ...
    threadEntityMemory = updateThreadEntityMemory(threadEntityMemory, extractRecordReferences(finalContent));
...
export function createGatewayChatRoutes(db, auth, env) {
    ...
    app.post("/start", authMiddleware(auth, db), async (c) => {
        ...
        const channel = typeof body === "object" &&
            body !== null &&
            "channel" in body &&
            body.channel === "voice"
            ? "voice"
    ...
    app.post("/", authMiddleware(auth, db), async (c) => {
        ...
        try {
            ...
            if (err.message === "No user message found") {
...
async function generateSmartTitle(db, threadId, queryText, gatewayHeaders, gatewayUrl) {
