...
function buildSystemPrompt(artifactDir: string): string {
  ...
  return [
    ...
    `When the user wants a downloadable FILE (report, csv, markdown, docx, pptx, xlsx, image, etc.), create it by RUNNING CODE (e.g. Python: os.makedirs("${artifactDir}", exist_ok=True) then write to "${artifactDir}/<name>") — do NOT use the file tool for this, and always use that absolute path. python-docx, python-pptx, and openpyxl are installed. Anything saved there is automatically offered to the user as a download — just tell them what you made; never paste file paths or base64.`,
