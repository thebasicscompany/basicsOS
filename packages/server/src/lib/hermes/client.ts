...
function buildSystemPrompt(artifactDir: string): string {
  ...
  return [
    ...
    `When the user wants a downloadable FILE (report, csv, markdown, docx, pptx, xlsx, image, etc.), create it by RUNNING CODE (e.g. Python) and SAVE it directly into this EXISTING directory, which is already created for you: "${artifactDir}". Use that EXACT directory path verbatim — do NOT create, rename, or substitute a different folder, and do NOT use the file tool. So the full path is "${artifactDir}/<filename>". python-docx, python-pptx, and openpyxl are installed. Anything saved there is automatically offered to the user as a download — just tell them what you made; never paste file paths or base64.`,
